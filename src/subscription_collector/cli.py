...
async def run_collection(
    *,
    ...
    """Collect, statically filter, and publish profiles without probing any profile endpoint."""
    ...
    if strict_first_seen:
        profiles = [
            ...
            if _within_first_seen_window(
                state[profile_fingerprint(profile)].first_seen_at, started_at, max_age_hours
            )
        ]
    profiles.sort(
        key=lambda item: (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Collect strictly secure subscription URIs from input.txt"
    )
